 ...
 * <p/>
 * Scans to indexed locations and reads profile identity records.
 *
 * @author ben <ben.vanberg@fullcontact.com>
 */
public class HadoopReadSomeSSTableData {
    ...
    private static final String CQL = "CREATE TABLE results (searched_value text, searched_type text, st_name text, version text, last_update timestamp, rawdata text, result text, statuscode text, statustext text, uri text, PRIMARY KEY (searched_value, searched_type, st_name, version)) WITH bloom_filter_fp_chance=0.000750 AND caching='ALL' AND comment='' AND dclocal_read_repair_chance=0.000000 AND gc_grace_seconds=864000 AND read_repair_chance=0.100000 AND replicate_on_write='true' AND populate_io_cache_on_flush='false' AND compaction={'class': 'SizeTieredCompactionStrategy'} AND compression={'sstable_compression': 'SnappyCompressor'}";

    // TODO: These hard coded types should be configured.
    public static final AbstractType keyType = UTF8Type.instance;
    private static final AbstractType columnNameConvertor = CompositeType.getInstance(Lists.<AbstractType<?>>newArrayList(UTF8Type.instance, UTF8Type.instance, UTF8Type.instance, UTF8Type.instance));

    public static void main(String[] args) throws IOException {

        HadoopReadSomeSSTableData readSomeSSTableData = new HadoopReadSomeSSTableData();
    ...
    private void readSomeDatasWoohoo() throws IOException {
        ...
        // Create a local filesystem for local testing.
        FileSystem fileSystem = FileSystem.getLocal(new Configuration());

        // Get the index
        SSTableIndex index = SSTableIndex.readIndex(fileSystem, new Path(INDEX_FILE));
        ...
        for (int i = 0; i < 10; i++) {
            ...
            final ByteBuffer keyBytes = ByteBufferUtil.readWithShortLength(reader);
//            System.out.println("Bytes[" + i + "]:\n" + keyBytes);

            final String key = keyType.getString(keyBytes);

            DecoratedKey decoratedKey = new RandomPartitioner().decorateKey(keyBytes);

            final long dataSize = offsets[i + 1] - reader.getFilePointer();
...
//            SSTableIdentityIterator ssTableIdentityIterator = new SSTableIdentityIterator(null, reader, decoratedKey, dataSize, false);

            final String jsonProbably = getMapperValue(ssTableIdentityIterator, null, cfMetaData).toString();

            System.out.println("JSON Probably: \n" + jsonProbably);
        }
    }

    public static Text getMapperValue(SSTableIdentityIterator rowIter, Mapper.Context context, CFMetaData cfMetaData) {
...
//        sb.append(", ");
    ...
    private static void serializeColumns(StringBuilder sb, SSTableIdentityIterator rowIter, Mapper.Context context, CFMetaData cfMetaData) {
