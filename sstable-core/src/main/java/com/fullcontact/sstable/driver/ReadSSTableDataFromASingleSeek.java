 ...
 */
public class ReadSSTableDataFromASingleSeek {
    ...
    public static final AbstractType keyType = CompositeType.getInstance(Lists.<AbstractType<?>>newArrayList(UTF8Type.instance, UTF8Type.instance));
    private static final AbstractType columnNameConvertor = CompositeType.getInstance(Lists.<AbstractType<?>>newArrayList(UTF8Type.instance, UTF8Type.instance, UTF8Type.instance));

    public static void main(String[] args) throws IOException {

        ReadSSTableDataFromASingleSeek readSomeSSTableData = new ReadSSTableDataFromASingleSeek();
    ...
    private void readSomeDatasWoohoo() throws IOException {
        ...
        // Create the reader
        CompressionMetadata compressionMetadata = CompressionMetadata.create(DATA_FILE, fileSystem);
        CompressedRandomAccessReader reader = CompressedRandomAccessReader.open(new Path(DATA_FILE), compressionMetadata, fileSystem);

        // Read from first index and continue. Is that possible?
        long offset = offsets[5];
        reader.seek(offset);
